...
def get_columns():
    columns = [
        {"label": (""), "fieldname": "name", "fieldtype": "Data", "width": 150},
        {
            "label": (""),
            "fieldname": "value",
            "fieldtype": "Currency",
...
def get_data(filters=None):
    ...
    net_sales = frappe.db.sql(
		...
		""".format(
            net_sales_condition=net_sales_condition
        )
    )
    row = {
        "name": "Net Sales",
    ...
    whole_net_sales = frappe.db.sql(
        """ SELECT Distinct
                SUM(sit.amount)
                FROM `tabSales Invoice` AS si,`tabSales Invoice Item` AS sit
                where {whole_net_sales_condition}
                """.format(
            whole_net_sales_condition=whole_net_sales_condition
        )
    )

    # 	frappe.msgprint(str(whole_net_sales))

    whole_collection_amount = frappe.db.sql(
        """ SELECT
    ...
    if filters.get("supplier_group"):
        customer = frappe.db.sql(
            """select name as name from `tabSupplier` where supplier_group = %s""",
    ...
    payable = frappe.db.sql(
        """ SELECT
                ...
                """.format(
            payable_condition=payable_condition
        )
    )
    row = {
...
def collection_conditions(filters):
    ...
    customer = []
    conditions += (
        "docstatus = 1 and excel_tax_payment = 'No' and payment_type = 'Receive' "
    )
    if filters.get("customer_group") and filters.get("territory"):
        customer = frappe.db.sql(
            """ SELECT
                ...
                """,
            (filters.get("customer_group"), filters.get("territory")),
            as_dict=1,
        )
    ...
    if filters.get("from_date"):
        conditions += " and posting_date >= '%s' " % filters.get("from_date")
    if filters.get("to_date"):
        conditions += " and posting_date <= '%s' " % filters.get("to_date")
...
def receivable_conditions(filters):
    ...
    if filters.get("to_date"):
        conditions += " and posting_date <= '%s' " % filters.get("to_date")
    if filters.get("customer_group"):
        conditions += " and customer_group = '%s' " % filters.get("customer_group")
    if filters.get("territory"):
        conditions += " and territory = '%s' " % filters.get("territory")
