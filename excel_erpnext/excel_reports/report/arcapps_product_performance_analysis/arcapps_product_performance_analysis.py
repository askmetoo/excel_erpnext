...
def _execute(
    filters=None, additional_table_columns=None, additional_query_columns=None
):
    if not filters:
        filters = {}
    filters.update(
...
def get_conditions(filters):
    ...
    for opts in (
        ...
        (
            "customer_group",
            " and `tabSales Invoice`.customer_group = %(customer_group)s",
        ),
        ("territory", " and `tabSales Invoice`.territory = %(territory)s"),
        ("sales_person", " and `tabSales Team`.sales_person = %(sales_person)s"),
...
def get_group_by_conditions(filters, doctype):
    if filters.get("group_by") == "Invoice":
        ...
        return "ORDER BY `tab{0} Item`.{1} , `tab{0}`.customer".format(
            doctype, frappe.scrub(filters.get("group_by"))
        )
        ...
        return "ORDER BY `tab{0}`.{1}".format(
            doctype, frappe.scrub(filters.get("group_by"))
        )
                        ...
                        if item_tax_amount:
                            ...
                            tax_value = (
                                ...
                                if (
                                    ...
                                    and name in deducted_tax
                                )
                                else tax_value
                            )

                            itemised_tax.setdefault(d.name, {})[
                                description
                            ] = frappe._dict(
...
def get_group_by_and_display_fields(filters):
    if filters.get("group_by") == "Item":
        ...
        subtotal_display_field = "item_group"
