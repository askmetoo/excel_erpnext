...
class PartyLedgerSummaryReport(object):
	...
	def get_data(self):
		...
		for gle in self.gl_entries:
			...
			if self.party_data[gle.party].credit_limit is not None:
				self.party_data[gle.party].remaining_balance = (self.party_data[gle.party].credit_limit) - (self.party_data[gle.party].closing_balance)
	...
	def get_gl_entries(self):
		...
		self.gl_entries = frappe.db.sql("""
			select
				...
				gle.against_voucher, gle.debit, gle.credit, ccl.credit_limit, gle.is_opening {join_field}
			from `tabGL Entry` gle LEFT JOIN `tabCustomer Credit Limit` ccl ON gle.party = ccl.parent
			{join}
			where
