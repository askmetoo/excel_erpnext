...
def get_conditions(filters):
	...
	for opts in (("company", " and company=%(company)s"),
		...
		("customer_group", " and `tabSales Invoice`.customer_group = %(customer_group)s"),
...
def get_group_by_conditions(filters, doctype):
	if filters.get("group_by") == 'Invoice':
		...
		return "ORDER BY `tab{0} Item`.{1} , `tab{0}`.customer".format(doctype, frappe.scrub(filters.get('group_by')))
